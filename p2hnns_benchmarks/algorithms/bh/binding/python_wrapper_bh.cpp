...
namespace p2h
{
	...
	{
	public:
		std::unique_ptr<Angular_Hash<float>> bh;

		void preprocess(int n, int d, int m, int l, float b, py::array_t<float> data)
		{
			py::buffer_info buf = data.request();
			float* ptr = static_cast<float*>(buf.ptr);
			bh.reset(new Angular_Hash<float>(n, d, 2, m, l, b, ptr));
		}
	...
	PYBIND11_MODULE(bh, m)
	{
		py::class_<WrapperBH>(m, "BH")
			.def(py::init<>())
			.def("preprocess", &WrapperBH::preprocess)
			.def("search", &WrapperBH::search);
	}
}