...
#if defined(MQH_ARM) && defined(MQH_NEON)
    // ARM NEON SIMD
    #include <arm_neon.h>
    #define USE_ARM_NEON 1
#elif defined(FORCE_X86_SIMD)
    // x86 SIMD
    #include <immintrin.h>  // AVX, AVX2
    #include <emmintrin.h>  // SSE2
    #define USE_X86_SIMD 1
#else
    // No SIMD - fallback to scalar
...
static float compare_ip(const float *a, const float *b, unsigned size) {
    // Log SIMD support on first call
    static bool first_call = true;
    if (first_call) {
#if defined(USE_ARM_NEON) && USE_ARM_NEON
        std::cout << "SIMD support: ARM NEON" << std::endl;
#elif defined(USE_X86_SIMD) && USE_X86_SIMD
        std::cout << "SIMD support: x86 (AVX/SSE2)" << std::endl;
#else
        std::cout << "SIMD support: None" << std::endl;
#endif
        first_call = false;
    }
    ...
    for (; i <= static_cast<int>(size) - step; i += step) {
...
#elif defined(USE_X86_SIMD) && USE_X86_SIMD
    // x86 SIMD implementation - try AVX first
    #ifdef __AVX__
