...
namespace p2h
{
	...
	{
	public:
		std::unique_ptr<Angular_Hash<float>> ah;

		void preprocess(int n, int d, int m, int l, float b, py::array_t<float> data)
		{
			py::buffer_info buf = data.request();
			float* ptr = static_cast<float*>(buf.ptr);
			ah.reset(new Angular_Hash<float>(n, d, 1, m, l, b, ptr));
		}
		...
		std::tuple<std::vector<int>, int> search(int top_k, int cand, py::array_t<float> query)
		{
			...
			int num_lin_scans = ah->nns(top_k, cand, ptr, &list);
	...
	PYBIND11_MODULE(ah, m)
	{
		py::class_<WrapperAH>(m, "AH")
			.def(py::init<>())
			.def("preprocess", &WrapperAH::preprocess)
			.def("search", &WrapperAH::search);
	}
}